...
def test_predict_probability(logistic_regression_model):
    ...
    y = np.array([0, 0, 0, 1, 1, 1])
    J_history = logistic_regression_model.fit(x, y, alpha=0.1, iterations=30, add_intercept=True)
...
def test_predict_with_classification(logistic_regression_model):
    x = np.array([1, 5, 8, 9, 10, 19])
    y = np.array([0, 0, 0, 1, 1, 1])
    J_history = logistic_regression_model.fit(x, y, alpha=0.1, iterations=1000, add_intercept=True)
